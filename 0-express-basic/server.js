...
// 우리의 웹 서버에 라우트 경로들을 등록한다. 라우트란, 쉽게 말해, ~ 경로로 클라이언트가 요청을 보내온다면,
// 어떤 처리를 한 뒤에, 어떤 답변을 할 것인가? 를 정의하는것이다.
// 따라서 클라이언트가 요청한 URL이 등록된 route가 아니라면, 적절히 처리되지 않는다. (404 Not found)

// 클라이언트의 URL 요청 중, "http://서버주소/public" 으로 시작하는 모든 URL 요청들은 
// 현재 디렉토리(__dirname) 내의, "public" 이라는 디렉토리 액세스로 맵핑을 한다.
// 즉, 예를 들어, http://서버주소/public/style.css 요청은 현재 디렉토리 내 "public" 폴더 내의 style.css 파일을 클라이언트에게 반환한다.
